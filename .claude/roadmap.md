...
- [ ] **[환경] Coil Checklist 템플릿 Excel 접근 불가 (2026-09-23 확인 · 2026-09-24 원인 규명)** — **원인: Excel COM이 `OneDrive …\Oxygen8 SharePoint Shortcuts\…` 경로를 못 연다**(파일은 안 열려 있음, openpyxl·로컬 복사본은 정상 오픈 확인). 수정안: `excel_writer._copy_template`이 Open 실패 시 바이트를 임시 파일로 복사해 여는 fallback(원본 무접촉) — John 승인 대기. 원문: — `OneDrive - Oxygen8\…\Production\Coil Checklist Template.xlsx`를 Excel COM이 "cannot access"(파일 자체는 읽힘, 12:59 수정). SharePoint 공동편집 잠금/동기화 확인 후 `python -m pytest tests/test_checklist_excel_writer.py -q`로 재확인(현재 11 skip).
