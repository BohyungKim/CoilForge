...
- [ ] **[환경] Coil Checklist 템플릿 Excel 접근 불가 (2026-09-23 확인)** — `OneDrive - Oxygen8\…\Production\Coil Checklist Template.xlsx`를 Excel COM이 "cannot access"(파일 자체는 읽힘, 12:59 수정). SharePoint 공동편집 잠금/동기화 확인 후 `python -m pytest tests/test_checklist_excel_writer.py -q`로 재확인(현재 11 skip).
...
## ⬜ 앞으로
- [ ] **[Terra 물코일 트랙] Terra HWC 템플릿 시딩** — 참조 `TERRA_HWC_LH/RH.pdf`가 바탕화면에 준비됨. CWC와 같은 절차(`TERRA_BUCKETS` + `extra_callouts`, `TERRA_TEMPLATES`, `_TERRA_WATER_WITHHELD_CATEGORIES`에서 HWC 제거 → 빈 집합이면 게이트 비활성). 커버리지 대시보드에 Terra 섹션도 추가(현재 shared + Ventum+만 모델링, `--check`는 shared만 검사).
- [ ] **[Terra 물코일 트랙] John 판단 — Terra CWC O 콜아웃 (2026-09-23)** — 새 아트워크의 O 치수선은 반대편 데이텀(참조 16.50 = CH − 2.75)인데 "값은 CoilForge 데이터" 결정대로 O = I(2.75)가 찍힘. 그대로 둘지, Terra CWC만 `O = CH − 2.75`(참조·시트와 일치, KD-024..027 은퇴)로 바꿀지. 현재 출하 상태 = O = I.
