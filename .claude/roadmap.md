...
## ✅ 완료
- [x] **[제출물 파싱 트랙] 2954 Aki Kurose — 커버 Casing 행 중복 코일 + Changeover 코일 제목 미인식 (2026-09-24, John 리포트)** — Oxygen8 v1.0.0.10 Ventum+ 제출물이 코일마다 `1 CCWC-1 CWC Cooling Casing` 부속 행을 같은 태그·Qty로 인쇄해 유령 코일이 되고, 그 행이 `_detail_lines_by_cover_row`의 순서 소비(`pop(0)`)로 다음 코일의 상세 블록까지 훔쳤다. 상세 페이지 제목 `Changeover Coil - Cooling Performance`는 `_detail_section_format`이 몰라 블록 0개 → FH/FL/Rows/FPI/Conn 전부 미추출 → CD/CH/S/R 공란. 수정: `_NON_COIL_ITEM_TOKENS`에 `casing`(전용 사유 문구 "not a second coil"), 제목 토큰을 `cooling_chilled_water`로, `Heating Performance`는 `Heating DX` 선례대로 stop. 실 PDF: 코일 2개, CCWC-1 FH27/FL72/IN·OUT 1.25 → CD 7.625/CH 29/S1 3.625/R2 1.25, CCWC-2 → CD 7.0/CH 27.5. 회귀 4건. plan-review R1 APPROVED. 범위 밖: CCWC-1(RH)은 `vplus_cwc_rh` 미시드로 공유 템플릿 폴백(문서화된 동작).
- [x] **[Terra 물코일 트랙] Terra CWC 전용 템플릿 시딩 + 물코일 S/R·CD 체크리스트 공식 (2026-09-23, John 지시)** — John이 CoilMaster로 만든 참조 `TERRA_CCWC_LH/RH`(`CW-A-F-06-10-18.00x36.00-L/R`)에서 `coilmaster_terra_cwc_{lh,rh}` 시딩(각 hand 자체 시드, 미러 없음). **Terra H + V 공용 아트워크**(`catalog.TERRA_TEMPLATES`, bucket family `TERRA`, alias `TERRA_H/TERRA_V/TERRA_H_C/"TERRA H C"`), 값은 전부 CoilForge 슬롯. 참조의 `4.00 HD1`·`10.00 SL1`은 시더의 알려진 구멍이라 버킷 한정 `extra_callouts`로 `slot.HD2/SL2`에 매핑(전역 `_DIM_LABELS` 무변경 — HGRH에서 SL1 의미가 달라서) → 박힌 치수 0. 게이트는 **카테고리 단위**(`_TERRA_WATER_WITHHELD_CATEGORIES = {"HWC"}`): CWC 해제, HWC 보류 유지(게이트가 dedicated-family 교체보다 먼저 돌아 SVG를 비우므로 카테고리 단위여야 함). **결정 ②·J-3a 종결(John)**: 전 제품군 물코일 CWC `S = IN/2+3`, `R = OUT` / HWC `S = IN`, `R = OUT`, CD = 체크리스트 R-071(`MAX(base, 접속경 항)`) — frozen 분석 경로가 IN/OUT을 안 넘기므로 비-frozen `_apply_water_connection_slots`가 추출값으로 재실행(추출값은 ManualOverride 아님, 친 값이 우선·감사), provenance는 `water_conn_applied` 미러로 drift 오보 없음. 실측: 2857 HHWC-1 CD 3.375→3.75. 테스트 10개는 결정 변경으로 재작성(약화 아님, 감사 확인), 신규 17개, 1760 green, invariant-guard BLOCKER/HIGH/MEDIUM 0. 눈확인 페이지: https://claude.ai/artifact/V8KZk7yme3arFNLvwbXqtJ
- [x] **[CCSI 트랙] 물코일 Drain and Vent Location 기본값 + 필러 css 셀렉터 수정 (38bc5fb, John 요청 2026-09-23)** — 값 하나(`app.js::CCSI_WATER_DRAIN_VENT_LOCATION`)가 물코일 미러 기본값(종전 `Stub/Connection`)과 새 top-level 페이로드 키 `drain_vent_location`(CWC/HWC만, `fields` 계약 밖 — Notes와 같은 이유)을 함께 구동. userscript는 **옵션 텍스트로** 선택(CCSI option value는 내부 코드; 공백·대소문자·끝 마침표 관용, 옵션 없으면 쓰지 않고 행 ⚠). 세 사본(app/userscript/ccsi-fill 스킬)은 테스트로 동일성 고정. **발견·수정**: `resolve()`에 css 분기가 없어 `#DrawingNotes`(2026-08-05 캡처)가 해석 불가 → labelText는 CCSI의 잘못된 `for="Drawing_Notes"`로 실패 → **Notes 푸시 무동작**이 커밋본으로 재현됨("selector not found"). 헤드리스 모의 폼에서 옵션 선택(value 코드 `4`)·미일치 무기록·Notes 채움 확인. userscript 2.2.3. 다른 세션의 userscript/app.js/style.css 미커밋 hunk는 격리(-U0 적용이 삽입 위치를 어긋나게 해 한 번 폐기 후 LF 정규화 + 3줄 문맥 patch로 재구성, 인덱스 export 1733 green).
- [x] **[TR-13 부분] 눈확인 스크린샷 (2026-09-23)** — 실 2808 + Terra V 온수(합성) 11장을 아티팩트로 공유(https://claude.ai/artifact/UrK6YxyJ1vvEn9t61BaUsz). 발견 1건(채택 행 `↙` 아이콘 누락) 수정·38bc5fb 포함. 남은 것: 켜진 자동채움에서 실제 재채움 후 표시, 실 Terra 물코일 제출물.
...
- [ ] **[TR-14] CCSI 물코일 fill 라이브 확인 + Drain and Vent Location select id 캡처 (John, CCSI 로그인 필요)** — Tampermonkey 2.2.3 설치 확인 → 물코일 1개 Send to CCSI → 패널 DVL 행의 ⚠ 대상이 실제 드롭다운인지 확인 → Fill → `Hdr Side In Airflow Dir.` 선택 + **Drawing Notes가 이제 실제로 채워지는지**(38bc5fb 이전엔 무동작). 그때 select `#id`를 캡처해 `selectors` 1순위 css + `selector_verified:true`로 플립(소커밋, Phase F 절차).
- [x] **[환경] Coil Checklist 템플릿 Excel 접근 불가 — 해소 (2026-09-24, John 승인)** — **원인: Excel COM이 `OneDrive …\Oxygen8 SharePoint Shortcuts\…` 경로를 못 연다**(파일은 안 열려 있음). `excel_writer._copy_template`에 바이트 복사 fallback(원본은 읽기만) + 복사 실패 시 격리 Excel 인스턴스 Quit(고아 EXCEL.EXE 누수 수정). Excel writer 테스트 **11/11 통과**(전날 10 skip). 원문: — `OneDrive - Oxygen8\…\Production\Coil Checklist Template.xlsx`를 Excel COM이 "cannot access"(파일 자체는 읽힘, 12:59 수정). SharePoint 공동편집 잠금/동기화 확인 후 `python -m pytest tests/test_checklist_excel_writer.py -q`로 재확인(현재 11 skip).
...
- [ ] **[체크리스트 정렬 트랙] Dave에게 시트 수정 요청 — RP-003 + RP-004 (2026-09-24)** — RP-003: ①HGRH Terra V O4/O6/O8=2 → John 판정 **Terra V 전 헤더 2.75 / Terra H 전 헤더 3.25**(CoilForge는 이미 그대로) ②SL5/SL7 `C47` 참조 → 요청 목록 포함 ③CWC NOTES `C327` → **09-24 템플릿에서 수정 확인**. RP-004(`docs/rule_proposals/RP-004-checklist-comparison-gaps.md`): 시트가 숫자를 안 주는 행(HGRH I3/I5/I7 항상 TBD, Ventum+ HGRH O TBD, 물코일 I/O/S/R/HD/OAL이 FEEDS 공란에 전부 공란), OMNIA 유닛 부재, Install B23 TBD, HGRH Terra V 명시 분기(저), DX/HGRH RB=1.75(규칙 1.5). **물코일 FEEDS 게이트 = John A(양쪽 제거, 2026-09-24)** — CoilForge 쪽 적용 완료(`_emit_cwc_io_hd_sl`: FEEDS 없어도 io/hd HIGH), 시트 쪽은 RP-004 A3로 Dave 요청. 시트가 고쳐지기 전까지 해당 행은 missing_one(체크리스트 쪽 공란)으로 뜸. CoilForge 쪽 후속: DX DIST ORIENTATION 비교 매핑 없음.
