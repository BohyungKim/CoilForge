...
| `X` | `mvp_required` | Bottom dimension table and selected water/HGRH drawings. | **Present only on header-connected coils.** Measured over 328 real HGRH pages (2026-09-05): a page whose `RETURN CONN SIZE` reads `… " OD Header` always prints `X` (186/186); one reading `… " swt` always leaves it blank (142/142), together with `I`/`S`/`O`/`R`. No exceptions. | **What sets the value is UNKNOWN.** The working reading is a header-stack depth `(h+1)*D + (h-1)*1.5` (same shape as `R-073`'s header-bank term, and `EZC-0016` has `CD` == `X` == 5.50), but it explains only 96 of the 186 valued pages (52%): the misses fall between the formula's steps at every integer h, and two coils identical in `FH·FL·CH·CL·CD·OAL·SL·I·S·O` differ in `X` (1.94 vs 2.00). The earlier "tube projection" reading was also wrong. **No engine rule; not derivable from the corpus** — an SOP/CoilMaster definition is needed. See `docs/wiki/concepts/x-header-stack-depth.md`. |
